...
function generateTrackList(folder, count) {
  const trackList = [];
  for (let i = 1; i <= count; i++) {
    trackList.push(`${folder}/track${i}.mp3`);
  }
  return trackList;
}
...
// Generate track lists for both light and dark music folders
const lightTrackList = generateTrackList('music/light', 24);
const darkTrackList = generateTrackList('music/dark', 30);

// Play music on page load
const musicPlayer = playMusic(lightTrackList);
