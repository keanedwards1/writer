...
const newPromptButton = document.getElementById("new-prompt-container");
...
submitContainer.addEventListener("mousemove", function(event) {
  ...
  submitTooltip.style.left = x - 40 + "px";
});

newPromptButton.addEventListener("mousemove", function(event) {
  const x = event.clientX;
  const y = event.clientY;
